    ...
    if (document.querySelector("." + object + "-added-descr") !== null) {
        ...
        for (let option of options) {
            if (option.innerHTML == descr) {
                ...
                alert(
                    "Ítem já adicionado, utilize o campo numérico para determinar a quantidade, digitando zero ou deixando o campo vazio o ítem será removido !",
                );
                return;
            }
        }
...
function toggleSelectAll(tecId) {
    // Seleciona todos os checkboxes específicos deste modal
    const checkboxes = document.querySelectorAll(`.client-checkbox-${tecId}`);
    ...
    // Opcional: Mudar o texto do botão dinamicamente
    const btn = event.target;
    btn.innerText = allChecked ? "Selecionar Todos" : "Desmarcar Todos";
}
